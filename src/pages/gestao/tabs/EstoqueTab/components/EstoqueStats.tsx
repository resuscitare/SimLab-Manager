...
export const EstoqueStats = ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Vencendo/Vencido</CardTitle>
          <AlertTriangle className="h-4 w-4 text-orange-600" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold text-orange-600">{vencendo + vencido}</div>
          <p className="text-xs text-gray-500">{vencido} já vencidos</p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold text-purple-600">R$ {valorTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
